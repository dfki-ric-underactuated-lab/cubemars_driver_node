...
class ODriveMotionController:
    ...
    def _apply_and_save_config(self):
        logging.info("--- PHASE 1: Applying Configuration (no save/reboot yet) ---")
        self.my_drive = odrive.find_any(self.drive_id) if self.drive_id else odrive.find_any()
        if not self.my_drive:
            logging.error("Could not find ODrive for configuration. Check connection/ID.")
            return False

        self.axis = self.my_drive.axis0

        try:
            self.my_drive.config.dc_bus_overvoltage_trip_level = 50
            self.my_drive.config.dc_bus_undervoltage_trip_level = 10.5
            self.my_drive.config.dc_max_positive_current = math.inf
            self.my_drive.config.dc_max_negative_current = -math.inf

            self.axis.config.motor.motor_type = MotorType.HIGH_CURRENT
            self.axis.config.motor.pole_pairs = 21
            self.axis.config.motor.torque_constant = 0.23628571428571427

            self.axis.config.motor.current_soft_max = 25
            self.axis.config.motor.current_hard_max = 30

            self.axis.config.motor.calibration_current = 10
            self.axis.config.motor.resistance_calib_max_voltage = 20
            self.axis.config.calibration_lockin.current = 15

            self.axis.motor.motor_thermistor.config.enabled = False

            # Controller / motion (limits in MOTOR units)
            self.axis.controller.config.vel_limit = VEL_MAX_TURNS_S
            self.axis.controller.config.vel_limit_tolerance = 1.2
            #self.axis.controller.config.vel_gain = 0.42
            #self.axis.controller.config.vel_interator_gain = 0.42
            ...
            self.axis.config.torque_soft_min = -math.inf
            self.axis.config.torque_soft_max = math.inf
            self.axis.trap_traj.config.accel_limit = 10

            # CAN and encoder
            self.my_drive.can.config.protocol = Protocol.SIMPLE
            self.my_drive.can.config.baud_rate = 250000
            self.axis.config.can.node_id = 63
            self.axis.config.can.heartbeat_msg_rate_ms = 100
            self.axis.config.enable_watchdog = False

            self.axis.config.load_encoder = EncoderId.SPI_ENCODER0
            self.axis.config.commutation_encoder = EncoderId.SPI_ENCODER0
            self.my_drive.spi_encoder0.config.mode = SpiEncoderMode.AMS
            self.my_drive.spi_encoder0.config.ncs_gpio = 17
            self.my_drive.config.enable_uart_a = False

            #logging.info("Starting calibration...")
            #self.axis.requested_state = AxisState.FULL_CALIBRATION_SEQUENCE
            #while self.axis.current_state != AxisState.IDLE:
                #time.sleep(0.1)
            #logging.info("Calibration complete, axis is IDLE.")

            return True

        except Exception as e:
            logging.error(f"Configuration Failed: {type(e).__name__}: {e}")
            return False
