...
namespace cubemars
{
    ...
    static const std::map<std::string, joint_config_t> joint_config_per_motor_type ={
        ...
        // MAB electronics (CAN FD register protocol, used by the MabFdCan backend). reply_on_own_id=true
        // so the RX filter matches the motor's own id. Ranges are inherited from AK10-9 and used as
        // command clamp bounds by MabFdCan - tune them to the motor and the MAB command units as needed.
